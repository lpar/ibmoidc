...
func makeTestClaimSet(t *testing.T) *jwt.ClaimSet {
	cs := jwt.NewClaimSet()

	err := cs.Construct(map[string]interface{}{
	...
	if err != nil {
		t.Error("Unable to construct test claimset")
	}

	err = cs.Set("aud", []string{"www.example.com"})
	if err != nil {
		t.Error("Unable to set audience array on test claimset")
	}
...
func checkTestClaimSet(t *testing.T, cs *jwt.ClaimSet) {
	...
	if cs.Get("given_name") != "Jason" {
		t.Errorf("Decoded id_token had wrong given_name, expected Jason got %s", fn)
	}

	ln := cs.Get("family_name")
	if ln != "Webb-Toucan" {
		t.Errorf("Decoded id_token had wrong family_name, expected Webb-Toucan got %s", ln)
	}
	cn := cs.Get("name")
	if cn != "Jason Webb-Toucan" {
		t.Errorf("Decoded id_token had wrong name, expected Jason Webb-Toucan got %s", cn)
	}
}
...
func TestDecode(t *testing.T) {
	...
	payload := makeTestClaimSet(t)

	jpay, err := payload.MarshalJSON()
	if err != nil {
		t.Errorf("Failed to marshal claimset payload: %s", err)
	}
	...
	if err != nil {
		t.Errorf("Error checking cryptographic signature on known good id_token: %s", err)
	}
	...
	_, err = jws.Verify(badtok, jwa.RS256, IBMw3idPublicKey)
	if err == nil {
